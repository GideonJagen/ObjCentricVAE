...
from objVAE.objVAE.MultiheadAttention_AE import MultiheadAttention
...
class MultiEntityVariationalAutoEncoder(pl.LightningModule):
    def __init__(
        ...
        decoder_latent_dim = latent_dim + 1
        ...
        elif decoder == "linnear":
            self.decoder_1 = nn.Sequential(
                nn.Linear(decoder_latent_dim, decoder_feature_size),
                nn.Mish(),
                nn.Linear(decoder_feature_size, decoder_feature_size),
                nn.Mish(),
                nn.Linear(decoder_feature_size, decoder_feature_size),
                nn.Mish(),
                nn.Linear(decoder_feature_size, decoder_feature_size),
    ...
    def forward(self, x):
        ...
        x_coord = (x_coord + 0.5) * x.shape[2] / y.shape[2] - xy_pred[:, :, 0]
        y_coord = (y_coord + 0.5) * x.shape[3] / y.shape[3] - xy_pred[:, :, 1]
        ...
        r_channel = torch.norm(grid_channel, p=2, dim=-1, keepdim=True)

        if self.object_radius:
            mask = torch.squeeze(r_channel < (self.object_radius))
            grid_channel = grid_channel[mask, :]
            latents = latents[mask, :]

            r_channel = torch.norm(grid_channel, p=2, dim=-1, keepdim=True)

        grid_channel = torch.cat([grid_channel, r_channel], dim=-1)

        # concatenate the grid channel and the latents
        # (batch_size, num_entities, latent_dim + 2, x.shape[2], x.shape[3])
        intermediate_feature_map = torch.cat([latents, grid_channel], dim=-1)
        ...
        if self.object_radius:
            ...
            pixel_map_tensor[mask] = decoded_feature_map.view(-1)

            decoded_feature_map = pixel_map_tensor

            mask_pres = (
                mask.view(batch_size, self.num_entities, 1, x.shape[3], x.shape[2])
                * pres
            )
        ...
        xy_coord = xy_coord.view(batch_size, self.num_entities, 2)
        pres = pres.view(batch_size, self.num_entities)
