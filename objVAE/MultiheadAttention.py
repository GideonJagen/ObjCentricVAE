...
class MultiheadAttention(pl.LightningModule):
    def __init__(
        self, max_t=1, softmax_factor=1, number_of_heads=1, num_filters=12, **kwargs
    ):
    ...
    def forward(self, latents, latents_frames):
        ...
        updated_latents, attention = self.multihead_time_attention(
            latents, self.softmax_factor, self.number_of_heads, mask
        )
    ...
    def multihead_time_attention(self, latents, softmax_factor, number_of_heads, mask):
        ...
        )  # Note: PyTorch's div function already handles division by zero
